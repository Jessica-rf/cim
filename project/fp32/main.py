...
from project.fp32.mnist_infer_fp32_hw_sim import *
from project.fp32.inst_gen import *
...
def write_data_to_file(file, M, mode="a"):
    ...
    with open(file, mode) as f:
        for arr in M:
            rows = math.ceil(len(arr) / 4)
            for r in range(rows):
                line = ""
                for s in range(4):
                    index = r * 4 + s
                    if index >= len(arr):
                        line += "00000000"  # 补齐不足16字节的部分
                    else:
                        line += f"{float_to_hex(arr[index])[2:]}"
                f.write(line + "\n")  # 写入每一行十六进制字符串
                cnt_line = cnt_line + 1
        if mode == "w":
...
def martix_mult(aa, bb, cc):
    data_bit = 32
    print(f"---> Cim_matrix_mult_test_fp32 ({aa},{bb})*({bb},{cc})", end=',  ')
    # d1 = np.arange(aa * bb, dtype=np.float32).reshape((aa, bb))
    # d2 = np.arange(bb * cc, dtype=np.float32).reshape((bb, cc))
    d1 = np.arange(aa * bb, dtype=np.float32).reshape((aa, bb)) % 11
    d2 = np.arange(bb * cc, dtype=np.float32).reshape((bb, cc)) % 13 - 6

    M0, M1 = cim_matrix_load(d1, d2)
    out, data = cim_matrix_mult_sim(M0, M1, d1.shape, d2.shape, data_bit, 0)
    ref = d1.dot(d2)
...
def martix_mult_with_output(aa, bb, cc):
    ...
    print(data)
    if np.array_equal(ref, out):
        print("---> Cim_matrix_mult_test_fp32 passed")
        inst = []
        GEN1 = InstGenerator()
        ...
        inst.extend(GEN1.gen_ldr_ptr("R", 8 + 8))
        inst.extend(GEN1.gen_ldr_ptr("T", 0))
        inst.extend(GEN1.gen_ldr_ptr("G", (8 + 8) + len(M0) * 8))
        ...
        print("---> Cim_matrix_mult_test_fp32 failed\n")
        return 1


def generate_random_numbers():
    while True:
        a = random.randint(1, 1024)  # a can be an integer
        b = random.randint(1, 199)  # b must be in the range (4, 200)
        c = random.randint(1, 1024)  # c can also be an integer
